...
#     radio = 0.6
#     constant = 16
#     center_distance = (((b1_x1 + b1_x2) - (b2_x1 + b2_x2)) ** 2 + ((b1_y1 + b1_y2) -
#                                                                    (b2_y1 + b2_y2)) ** 2) / 4
#     w1_n = w1 + eps
#     h1_n = h1 + eps
#     w2_n = w2 + eps
#     h2_n = h2 + eps
#
#     wh_distance = ((w1_n - w2_n) ** 2 + (h1_n - h2_n) ** 2) / 4
...
class BIoU_Scale:
    ...
    @classmethod
    def _scaled_loss(cls, self, λ=2.6, μ=2.1):
        if isinstance(self.monotonous, bool):
            if self.monotonous:
                return (self.iou.detach() / self.iou_mean).sqrt()
            else:
                L = self.iou.detach() / self.iou_mean
                alpha = (math.sqrt(μ)) * torch.pow(λ, L - μ)
                return L / alpha
        return 1


def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, ABIoU=False, Focal=False,
             alpha=1, gamma=0.5, scale=False, eps=1e-7):
    # Returns Intersection over Union (IoU) of box1(1,4) to box2(n,4)
    ...
    if scale:
        BIOU = 1 - (radio * (inter / union) + (1 - radio) * nwd)
        self = BIoU_Scale(BIOU)

    # IoU
    # iou = inter / union # ori iou

    iou = torch.pow(inter / (union + eps), alpha)  # alpha iou
    if CIoU or DIoU or GIoU or EIoU or SIoU or ABIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EIoU or SIoU or ABIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = (cw ** 2 + ch ** 2) ** alpha + eps  # convex diagonal squared
            rho2 = (((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (
                        b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4) ** alpha  # center dist ** 2
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if Focal:
                    ...
                    # 梯度增益，1-biou，iou
                    return getattr(BIoU_Scale, '_scaled_loss')(self), (1 - (radio*iou + (1-radio)*nwd)), iou  # WIoU https://arxiv.org/abs/2301.10051
                else:
                    return iou, torch.exp((rho2 / c2))  # WIoU v1
            if Focal:
                return iou - rho2 / c2, torch.pow(inter / (union + eps), gamma)  # Focal_DIoU
    ...
    if Focal:
        ...
        return radio*iou + (1-radio)*nwd  # BIOU
        # return nwd  # NWD
